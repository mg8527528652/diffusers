...
def prepare_train_dataset(dataset, accelerator):
    ...
    def preprocess_train(examples):
        ...
        # create single channel black images with same size as images
        images_black = [Image.new("L", image.size, (0)) for image in images]
        images_black = [resize_with_padding_pil(image, (args.resolution, args.resolution)) for image in images_black]
        images_black = [conditioning_image_transforms(image) for image in images_black]
        
        images = [resize_with_padding_pil(image, (args.resolution, args.resolution)) for image in images]
        images = [image_transforms(image) for image in images]

        # conditioning_images = [image.convert("RGB") for image in examples[args.conditioning_image_column]]
        # conditioning_images = [conditioning_image_transforms(image) for image in conditioning_images]
        conditioning_images = [image.convert("RGBA") for image in examples[args.conditioning_image_column]]
        conditioning_images = [resize_with_padding_pil(image, (args.resolution, args.resolution)) for image in conditioning_images]
        conditioning_images = [conditioning_image_transforms(image) for image in conditioning_images]
        # make alpha channel of conditioning_images binary. conditioning images contains tensors
        for i in range(len(conditioning_images)):
            alpha_channel = conditioning_images[i][3]  # Extract the alpha channel
            alpha_channel[alpha_channel >= 0.5] = 1
            alpha_channel[alpha_channel < 0.5] = 0  # Set background to 0
            conditioning_images[i][3] = alpha_channel  # Update the alpha channel
            # add black image as 5th channel to conditioning_images.
            conditioning_images[i] = torch.cat((conditioning_images[i], images_black[i]), dim=0)
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet = ControlNetModel.from_pretrained(args.controlnet_model_name_or_path, conditioning_channels=5)
    else:
