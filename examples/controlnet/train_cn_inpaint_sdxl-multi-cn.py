...
def log_validation(vae, unet, controlnet, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        mask = Image.open(validation_image).split()[-1]
        # masked_image = prepare_mask_and_masked_image(validation_image, mask)
        ...
        for _ in range(args.num_validation_images):
            with autocast_ctx:
                image = pipeline(
                    prompt=validation_prompt, image=validation_image, mask=mask,  num_inference_steps=20, generator=generator
                ).images[0]
            images.append(image)
...
def prepare_train_dataset(dataset, accelerator):
    ...
    def preprocess_train(examples):
        ...
        conditioning_images = [image for image in examples[args.conditioning_image_column]]
...
def collate_fn(examples):
    ...
    bg_masks = [example["conditioning_pixel_values"].split()[-1].convert("RGB") for example in examples]
    bg_masks = [ImageOps.invert(example) for example in bg_masks]
    conditioning_images = [example["conditioning_pixel_values"] for example in examples]
    # remove bg from 3channels
    conditioning_images = [convert4ch_to_3ch(example) for example in conditioning_images]
    # create alpha again. image from conditioning_images is a PIL image, and get mask from bg_masks. invert mask to get alpha
    conditioning_images = [Image.merge("RGBA", (*example.split()[:3], ImageOps.invert(bg_mask.convert("L") ))) for example, bg_mask in zip(conditioning_images, bg_masks)]
    # # pad
    conditioning_images = [resize_with_padding_pil(example, (args.resolution, args.resolution)) for example in conditioning_images]
    ...
    for i in range(len(pixel_values)):
        ...
        bg_masks[i] = Image.fromarray(mask)
        mask, masked_image = prepare_mask_and_masked_image(pixel_values[i], bg_masks[i])
        masks.append(mask)
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet = ControlNetModel.from_pretrained(args.controlnet_model_name_or_path, conditioning_channels=4)
    else:
    ...
    min_loss = torch.tensor(float('inf')).to(accelerator.device)  # Initialize as tensor with infinity
    image_logs = None
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                if loss.item() < min_loss.item():  # Compare float values
                    min_loss = loss.detach().clone()  # Store detached copy of loss tensor
                    # save checkpoint with name best_ckpt.pth   
                    save_path = os.path.join(args.output_dir, f"best_ckpt.pth")
                    accelerator.save_state(save_path)
                    # save a txt file with the loss value and step number
                    with open(os.path.join(args.output_dir, f"best_loss.txt"), "w") as f:
                        f.write(f"Loss: {loss.item()}, Step: {global_step}")
